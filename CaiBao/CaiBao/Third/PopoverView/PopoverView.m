...
- (void)showToView:(UIView *)pointView withTitleArray:(NSArray *)titleArray clickBlock:(ClickBlock)clickBlock
{
    ...
    for (int i = 0; i < titleArray.count; i++) {
        PopoverAction *action = [PopoverAction actionWithTitle:titleArray[i]];
        [array addObject:action];
    }
    
    [self showToView:pointView withArray:array];
}
...
- (void)showToView:(UIView *)pointView withTitleArray:(NSArray *)titleArray ImageArray:(NSArray *)imageArray clickBlock:(ClickBlock)clickBlock
{
    ...
    NSMutableArray *array = [NSMutableArray new];
    for (int i = 0; i < titleArray.count; i++) {
        PopoverAction *action = [PopoverAction actionWithImage:imageArray[i] title:titleArray[i]];
        [array addObject:action];
    }
    [self showToView:pointView withArray:array];
}

- (void)showToView:(UIView *)pointView withArray:(NSArray *)array
{
...
- (void)showToPoint:(CGPoint)toPoint withTitleArray:(NSArray *)titleArray clickBlock:(ClickBlock)clickBlock
{
    ...
    [self showToPoint:toPoint withArray:array];
}
...
- (void)showToPoint:(CGPoint)toPoint withArray:(NSArray *)array
{
    _actions = [array copy];
    // 计算箭头指向方向
